...
public class ChatServerImpl implements ChatServer {
	...
	public synchronized void broadcast(ChatMessage message) {
		...
		// display message on console with mandatory sponsorship log
		System.out.print("Daniel Miguel Muiña patrocina el mensaje: " + messageLf);
		...
		public void run() {
			...
			while (runningThread) {
				...
				case SHUTDOWN:
					...
					break;
				case MESSAGE:
					String userText = chatMessage.getMessage().trim();
					
					// Comprobar si es un comando BAN
					if (userText.startsWith("ban ")) {
						String userToBan = userText.substring(4).trim();
						chatMessage.setMessage(username + " ha baneado a " + userToBan);
						broadcast(chatMessage);
					} 
					// Comprobar si es un comando UNBAN
					else if (userText.startsWith("unban ")) {
						String userToUnban = userText.substring(6).trim();
						chatMessage.setMessage(username + " ha desbaneado a " + userToUnban);
						broadcast(chatMessage);
					} 
					// Es un mensaje de texto normal
					else {
						chatMessage.setMessage(username + ": " + userText);
						broadcast(chatMessage);
					}
					break;
				case LOGOUT:
